...
void TouchItemDispatcher(int client_id, const char* item_classname) {
    if (!touch_item_handler)
        return; // No registered handler.

    PyGILState_STATE gstate = PyGILState_Ensure();

    PyObject* result = PyObject_CallFunction(touch_item_handler, "is", client_id, item_classname);
