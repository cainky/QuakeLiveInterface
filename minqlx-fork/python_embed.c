...
PyObject* touch_item_handler = NULL;
...
// Get raw entity information for item/entity tracking
static PyObject* PyMinqlx_GetEntityInfo(PyObject* self, PyObject* args) {
    int ent_id;

    // Parse arguments: Expecting an integer (Entity ID)
    if (!PyArg_ParseTuple(args, "i", &ent_id)) {
        return NULL;
    }

    // Safety checks
    if (ent_id < 0 || ent_id >= MAX_GENTITIES) {
        Py_RETURN_NONE;
    }

    gentity_t *ent = &g_entities[ent_id];

    // If entity is not in use, return None
    if (!ent->inuse) {
        Py_RETURN_NONE;
    }
...
static PyMethodDef minqlxMethods[] = {
    ...
    {"get_entity_info", PyMinqlx_GetEntityInfo, METH_VARARGS,
     "Gets raw entity data by entity ID for item/entity tracking."},
    {NULL, NULL, 0, NULL}
};
