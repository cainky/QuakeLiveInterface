...
class QuakeLiveClientTest(unittest.TestCase):
    ...
    def send_command_test_helper(self, method, expected_command):
        method()
        self.assertEqual(self.client.connection.socket.sendto.call_args[0][0].decode(), expected_command)
    ...
    def test_record_demo(self):
        demo_name = "test_demo"
        self.send_command_test_helper(lambda: self.client.record_demo(demo_name), f"record {demo_name}")

    def test_say(self):
        message = "Hello, world!"
        self.send_command_test_helper(lambda: self.client.say(message), f"say {message}")

    def test_say_team(self):
        message = "Hello, team!"
        self.send_command_test_helper(lambda: self.client.say_team(message), f"say_team {message}")

    def test_voice_chat(self):
        voice_command = "test_command"
        self.send_command_test_helper(lambda: self.client.voice_chat(voice_command), f"voice_chat {voice_command}")
